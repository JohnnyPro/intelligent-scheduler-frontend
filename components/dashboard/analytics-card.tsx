...
export function RoomUtilizationCard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { success, data: utilData } = await repository.getRoomUtilization();
        if (success && utilData) {
          setData(utilData);
        }
      } catch (error) {
        ...
        data: [
          {
            ...
            itemStyle: {
              color: data && data.overall > 80 ? '#EF4444' : 
                     data && data.overall > 60 ? '#F59E0B' : '#10B981'
            }
          }
        ],
        detail: {
        ...
        {data && data.byBuilding.length > 0 && (
          <div className="mt-4 space-y-2">
            ...
            {data.byBuilding.slice(0, 3).map((building) => (
              <div key={building.buildingId} className="flex items-center justify-between text-sm">
                <span className="text-gray-600">{building.buildingName}</span>
                <span className="font-medium">{building.utilization.toFixed(1)}%</span>
              </div>
            ))}
          </div>
        )}
...
export function ScheduleQualityCard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { success, data: qualityData } = await repository.getScheduleQuality();
      ...
      <CardHeader>
        ...
        {data && (
          <p className="text-sm text-gray-500">
            Overall Score: <span className="font-semibold">{data.overallScore.toFixed(1)}%</span>
          </p>
        )}
      </CardHeader>
      <CardContent>
