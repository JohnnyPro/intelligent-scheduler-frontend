    ...
    <DashboardLayout title="Student Groups">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pb-2 border-b">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Manage student groups and their requirements</p>
          </div>
          <Button
          ...
          <Table>
            ...
            <TableBody>
              {filteredStudentGroups.map((group) => (
                <TableRow key={group.studentGroupId} className="hover:bg-muted/40">
                  <TableCell className="flex items-center gap-3 font-medium">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">ID: {group.studentGroupId}</div>
                    </div>
                  </TableCell>
                  <TableCell>{group.size}</TableCell>
                  <TableCell>
                    <div className="font-medium">{group.department?.name || 'N/A'}</div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => openConfirmDeleteDialog(group.studentGroupId)}
                        aria-label="Delete"
                      >
      ...
      <Dialog open={isAddDialogOpen || isEditDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl w-full">
          <DialogHeader>
            <DialogTitle>{isAddDialogOpen ? 'Add New Student Group' : 'Edit Student Group'}</DialogTitle>
          </DialogHeader>
          <form
            onSubmit={e => {
              e.preventDefault();
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-4 mt-5">
              ...
              <div className="flex items-center gap-2 md:col-span-2 pt-2">
                <Checkbox
                  ...
                  onCheckedChange={checked => setFormData({ ...formData, accessibilityRequirement: checked === true })}
                />
                <Label htmlFor="accessibilityRequirement" className="text-sm font-medium">Accessibility Requirements</Label>
              </div>
            </div>
