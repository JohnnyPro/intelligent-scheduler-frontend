...
interface StoreState {
  ...
  assignTeacher: (id: string, teacher: TeacherUpdating) => void
  unassignTeacher: (teacherId: string, courseIds: string[]) => void
...
export const useTeacherStore = create<StoreState>()(
  persist(
    (set, get) => ({
      ...
      assignTeacher: async (id, teacher) => {
        set({ isLoading: true })
        await toast.promise(repository.assignTeacher(id, {
          teacherId: id,
          ...
          error: (e) => {
            let userFriendlyMessage = "An unexpected error occurred while assigning teacher."
            if (e instanceof Error) {
              userFriendlyMessage = e.message
            }
            set({ isLoading: false })
      ...
      unassignTeacher: async (teacherId, courseIds) => {
        set({ isLoading: true })
        await toast.promise(repository.unassignTeacher(teacherId, courseIds), {
          loading: "Unassigning...",
          success: () => {
            get().fetchTeachers()
          ...
          error: (e) => {
            let userFriendlyMessage = "An unexpected error occurred while unassigning teacher."
            if (e instanceof Error) {
              userFriendlyMessage = e.message
            }
            set({ isLoading: false })
